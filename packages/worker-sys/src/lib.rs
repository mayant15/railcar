...
use libafl_bolts::shmem::{ShMemDescription, ShMemProvider, UnixShMem, UnixShMemProvider};
...
#[inline]
fn get_total_mut_ptr(shmem: &mut UnixShMem) -> *mut u32 {
    shmem.as_mut_ptr().cast::<u32>()
}

#[inline]
fn get_map_mut_slice(shmem: &mut UnixShMem) -> &mut [u8] {
    &mut shmem[4..]
}
...
impl CoverageMap {
    ...
    pub fn new(env: Env, desc: JsObject) -> napi::Result<Self> {
        ...
        let mut provider = UnixShMemProvider::new().unwrap();
