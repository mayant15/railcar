...
export class FuzzedDataProvider {
    ...
    consumeBigIntegral(maxNumBytesToConsume: number, isSigned = false): bigint {
        ...
        return this.consumeBigIntegralLEorBE(
            maxNumBytesToConsume,
            isSigned,
            true,
        );
    }
    ...
    consumeFloatInRange(min: number, max: number): number {
        ...
        if (
            min < 0.0 &&
            max > 0.0 &&
            max > min + FuzzedDataProvider.max_float
        ) {
    ...
    consumeDoubleInRange(min: number, max: number): number {
        ...
        if (
            min < 0.0 &&
            max > 0.0 &&
            max > min + FuzzedDataProvider.max_double
        ) {
            range = max / 2.0 - min / 2.0;
            if (this.consumeBoolean()) {
                result += range;
    ...
    consumeIntegrals(
        ...
        const arrayLength = this.computeArrayLength(
            maxLength,
            numBytesPerIntegral,
        );
    ...
    pickValues<Type>(array: Array<Type>, numValues: number): Array<Type> {
        ...
        for (let i = 0; i < numValues; i++) {
            const index = this.consumeIntegralInRange(
                0,
                remainingArray.length - 1,
            );
    ...
    private consumeBigIntegralInRangeLEorBE(
        ...
        while (range >> offset > BigInt(0) && this._remainingBytes > 0) {
            ...
            if (isLittleEndian) {
                nextByte = BigInt(
                    this.data[this.dataPtr + this._remainingBytes],
                );
            } else {
