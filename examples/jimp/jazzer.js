...
module.exports.fuzz = async function (data) {
    try {
        ...
        Jimp.read(jimpInput, (err, image) => {
            ...
            const width = provider.consumeIntegralInRange(
                0,
                image.bitmap.width,
            );
            const height = provider.consumeIntegralInRange(
                0,
                image.bitmap.height,
            );
            ...
            const fontX = provider.consumeIntegralInRange(
                0,
                image.bitmap.width,
            );
            const fontY = provider.consumeIntegralInRange(
                0,
                image.bitmap.height,
            );
...
function ignoredError(error) {
    return Boolean(
        ignored.find((message) => error.message.indexOf(message) !== -1),
    );
}
