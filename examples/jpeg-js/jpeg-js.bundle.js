            ...
            function initQuantTables(sf) {
                ...
                for (var row = 0; row < 8; row++) {
                    for (var col = 0; col < 8; col++) {
                        ...
                        fdtbl_UV[k] =
                            1 /
                            (UVTable[ZigZag[k]] * aasf[row] * aasf[col] * 8);
                        k++;
                    }
                }
            }
...
var require_decoder = __commonJS({
    "lib/decoder.js"(exports, module) {
        var JpegImage = (function jpegImage() {
            ...
            function decodeScan(
                    ...
                    while (k2 <= e) {
                        ...
                        switch (successiveACState) {
                            ...
                            case 2:
                                if (zz[z])
                                    zz[z] +=
                                        (readBit() << successive) * direction;
                                else {
                                    r--;
                                    if (r === 0)
                                        successiveACState =
                                            successiveACState == 2 ? 3 : 0;
                                }
                                break;
                            case 3:
                                if (zz[z])
                ...
                function decodeMcu(component2, decode2, mcu2, row, col) {
                    ...
                    if (
                        component2.blocks[blockRow] === void 0 &&
                        opts.tolerantDecoding
                    )
                        return;
            ...
            function buildComponentData(frame, component) {
                ...
                function quantizeAndInverse(zz, dataOut, dataIn) {
                    ...
                    var p = dataIn;
                    var i2;
                    for (i2 = 0; i2 < 64; i2++) p[i2] = zz[i2] * qt[i2];
                    for (i2 = 0; i2 < 8; ++i2) {
                        var row = 8 * i2;
                        if (
                            p[1 + row] == 0 &&
                    ...
                    for (i2 = 0; i2 < 8; ++i2) {
                        ...
                        v3 = p[6 * 8 + col];
                        v4 =
                            (dctSqrt1d2 * (p[1 * 8 + col] - p[7 * 8 + col]) +
                                2048) >>
                            12;
                        v7 =
                            (dctSqrt1d2 * (p[1 * 8 + col] + p[7 * 8 + col]) +
                    ...
                    for (i2 = 0; i2 < 64; ++i2) {
                        ...
                        dataOut[i2] =
                            sample2 < 0 ? 0 : sample2 > 255 ? 255 : sample2;
                    }
                }
            ...
            constructor.prototype = {
                ...
                parse: function parse(data) {
                    var maxResolutionInPixels =
                        this.opts.maxResolutionInMP * 1e3 * 1e3;
                    var offset = 0,
                        length = data.length;
                    function readUint16() {
                    ...
                    function prepareComponents(frame2) {
                        ...
                        for (componentId2 in frame2.components) {
                            if (
                                frame2.components.hasOwnProperty(componentId2)
                            ) {
                        ...
                        var mcusPerLine = Math.ceil(
                            frame2.samplesPerLine / 8 / maxH2,
                        );
                        var mcusPerColumn = Math.ceil(
                            frame2.scanLines / 8 / maxV2,
                        );
                        for (componentId2 in frame2.components) {
                            if (
                                frame2.components.hasOwnProperty(componentId2)
                                ...
                                var blocksPerLine = Math.ceil(
                                    (Math.ceil(frame2.samplesPerLine / 8) *
                                        component2.h) /
                                        maxH2,
                                );
                                var blocksPerColumn = Math.ceil(
                                    (Math.ceil(frame2.scanLines / 8) *
                                        component2.v) /
                                        maxV2,
                                );
                                var blocksPerLineForMcu =
                                    mcusPerLine * component2.h;
                                var blocksPerColumnForMcu =
                                    mcusPerColumn * component2.v;
                                var blocksToAllocate =
                                    blocksPerColumnForMcu * blocksPerLineForMcu;
                                var blocks = [];
                                requestMemoryAllocation(blocksToAllocate * 256);
                                for (
                                    var i2 = 0;
                                    i2 < blocksPerColumnForMcu;
                                    i2++
                                ) {
                                    var row = [];
                                    for (
                                        var j2 = 0;
                                        j2 < blocksPerLineForMcu;
                                        j2++
                                    )
                    ...
                    while (fileMarker != 65497) {
                        ...
                        switch (fileMarker) {
                            ...
                            case 65534:
                                ...
                                if (fileMarker === 65534) {
                                    var comment = String.fromCharCode.apply(
                                        null,
                                        appData,
                                    );
                                ...
                                if (fileMarker === 65505) {
                                    if (
                                        ...
                                        this.exifBuffer = appData.subarray(
                                            5,
                                            appData.length,
                                        );
                                    }
                                ...
                                if (fileMarker === 65518) {
                                    if (
                                        ...
                                        adobe = {
                                            version: appData[6],
                                            flags0:
                                                (appData[7] << 8) | appData[8],
                                            flags1:
                                                (appData[9] << 8) | appData[10],
                            ...
                            case 65474:
                                ...
                                var pixelsInFrame =
                                    frame.scanLines * frame.samplesPerLine;
                            ...
                            case 65476:
                                ...
                                for (i = 2; i < huffmanLength; ) {
                                    ...
                                    var huffmanValues = new Uint8Array(
                                        codeLengthSum,
                                    );
                                    for (
                                        j = 0;
                    ...
                    for (var i = 0; i < frames.length; i++) {
                        ...
                        for (var j in cp) {
                            cp[j].quantizationTable =
                                quantizationTables[cp[j].quantizationIdx];
                        ...
                        case 4:
                            for (y = 0; y < height; y++) {
                                for (x = 0; x < width; x++) {
                                    ...
                                    R =
                                        255 -
                                        clampTo8bit(C * (1 - K / 255) + K);
                                    G =
                                        255 -
                                        clampTo8bit(M * (1 - K / 255) + K);
                                    B =
                                        255 -
                                        clampTo8bit(Y * (1 - K / 255) + K);
                                    imageDataArray[j++] = R;
                                    imageDataArray[j++] = G;
                                    imageDataArray[j++] = B;
                                    if (formatAsRGBA) {
            ...
            function requestMemoryAllocation(increaseAmount = 0) {
                var totalMemoryImpactBytes =
                    totalBytesAllocated + increaseAmount;
                if (totalMemoryImpactBytes > maxMemoryUsageBytes) {
                    var exceededAmount = Math.ceil(
                        (totalMemoryImpactBytes - maxMemoryUsageBytes) /
                            1024 /
                            1024,
                    );
                    throw new Error(
        ...
        function decode(jpegData, userOpts = {}) {
            ...
            decoder.opts = opts;
            JpegImage.resetMaxMemoryUsage(
                opts.maxMemoryUsageInMB * 1024 * 1024,
            );
            decoder.parse(arr);
            ...
            try {
                ...
                if (err instanceof RangeError) {
                    throw new Error(
                        "Could not allocate enough memory for the image. Required: " +
                            bytesNeeded,
                    );
                }
