...
module.exports.fuzz = function (data) {
    try {
        ...
        const text = provider.consumeString(
            provider.consumeIntegralInRange(0, 10000),
        );
        ...
        while (scanner.scan() !== ts.SyntaxKind.EndOfFileToken) {
            ...
            if (
                provider.remainingBytes > 0 &&
                provider.consumeProbabilityFloat() < 0.1
            ) {
                scanner.resetTokenState(
                    provider.consumeIntegralInRange(0, text.length),
                );
            }
        }
    } catch (error) {
