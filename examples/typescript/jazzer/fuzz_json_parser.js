...
module.exports.fuzz = function (data) {
    ...
    const basePath = provider.consumeString(
        provider.consumeIntegralInRange(1, 100),
    );
    const existingOptions = createExistingOptions(provider);
    const configFileName = provider.consumeString(
        provider.consumeIntegralInRange(1, 100),
    );
    let json;
    try {
...
function createExistingOptions(provider) {
    return {
        target: provider.consumeIntegralInRange(
            ts.ScriptTarget.ES3,
            ts.ScriptTarget.Latest,
        ),
        module: provider.consumeIntegralInRange(
            ts.ModuleKind.None,
