...
module.exports.fuzz = async function (data) {
    ...
    try {
        ...
        if (shouldRemoveNode) {
            ...
            if (nodes.length > 0) {
                const indexToRemove = provider.consumeIntegralInRange(
                    0,
                    nodes.length - 1,
                );
        ...
        if (shouldAddNode) {
            ...
            ts.forEachChild(sourceFile, (node) => nodes.push(node));
            if (nodes.length > 0) {
                const indexToAddAt = provider.consumeIntegralInRange(
                    0,
                    nodes.length - 1,
                );
        ...
        for (let i = 0; i < numNodes; i++) {
            ...
            ts.forEachChild(sourceFile, (node) => nodes.push(node));
            if (nodes.length > 0) {
                const indexToReplace = provider.consumeIntegralInRange(
                    0,
                    nodes.length - 1,
                );
