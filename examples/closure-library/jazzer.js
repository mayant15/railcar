    ...
    try {
        ...
        switch (method) {
            ...
            case 2:
                ...
                break;
            case 3:
                safeHtml = SafeHtml.fromTrustedResourceUrl(
                    TrustedResourceUrl.fromConstant(googString.from(html)),
                );
                break;
            case 4:
                safeHtml = SafeHtml.fromUntrustedResourceUrl(
                    UncheckedResourceUrl.fromConstant(googString.from(html)),
                );
                break;
            case 5:
                safeHtml = SafeHtml.unwrap(
                    SafeHtml.create(tagName, { innerHtml: html }),
                );
                break;
            case 6:
                safeHtml = SafeHtml.concat(
                    SafeHtml.create(tagName, { innerHtml: html }),
                    SafeHtml.EMPTY,
                );
                break;
        ...
        const trustedSafeHtml = isTrusted
            ? SafeHtml.htmlEscape(htmlString)
            : concatenatedSafeHtml;
...
const ignored = ["Cannot read properties of"];
