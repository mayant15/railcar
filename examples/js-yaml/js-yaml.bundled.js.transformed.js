...
((global, factory) => {
  ...
  common.isObject = isObject_1;
  common.toArray = toArray_1;
  common.repeat = repeat_1;
  common.isNegativeZero = isNegativeZero_1;
  common.extend = extend_1;
  ...
  var int = new type("tag:yaml.org,2002:int", {
    ...
    represent: {
      ...
      octal: obj => obj >= 0 ? "0o" + obj.toString(8) : "-0o" + obj.toString(8).slice(1),
  ...
  function generateError(state, message) {
    ...
    mark.name = state.filename;
    mark.buffer = state.input.slice(0, -1);
    mark.position = state.position;
    mark.line = state.line;
    mark.column = state.position - state.lineStart;
  ...
  directiveHandlers.YAML = function handleYamlDirective(state, name, args) {
    var match, major, minor;
    if (state.version !== null) {
      throwError(state, "duplication of %YAML directive");
    }
  ...
  directiveHandlers.TAG = function handleTagDirective(state, name, args) {
    var handle, prefix;
    if (args.length !== 2) {
      throwError(state, "TAG directive accepts exactly two arguments");
    }
  ...
  jsYaml.FAILSAFE_SCHEMA = FAILSAFE_SCHEMA;
  jsYaml.JSON_SCHEMA = JSON_SCHEMA;
  jsYaml.CORE_SCHEMA = CORE_SCHEMA;
  jsYaml.DEFAULT_SCHEMA = DEFAULT_SCHEMA;
  jsYaml.load = load;
  jsYaml.loadAll = loadAll;
  jsYaml.dump = dump;
  jsYaml.YAMLException = YAMLException;
  jsYaml.types = types;
  jsYaml.safeLoad = safeLoad;
  jsYaml.safeLoadAll = safeLoadAll;
